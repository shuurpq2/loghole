...
SqliteDBLogger::SqliteDBLogger(std::string file_path) 
...
void SqliteDBLogger::m_log(Log& log) {
    const char* table_creation_sql = "CREATE TABLE IF NOT EXISTS logs(id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp TEXT, level TEXT, info TEXT);";

    if (sqlite3_exec(m_db, table_creation_sql, nullptr, nullptr, nullptr) != SQLITE_OK) {
        std::cerr << "Error: sqlite table creation error";
    }
    ...
    if (sqlite3_prepare_v2(m_db, log_insertion_sql, -1, &log_insertion_stmt, nullptr) != SQLITE_OK) {
        std::cerr << "Error: sqlite preparing error";
    }
    ...
    if (sqlite3_step(log_insertion_stmt) != SQLITE_DONE) {
        std::cerr << "Error: sqlite insertion error";
    }
